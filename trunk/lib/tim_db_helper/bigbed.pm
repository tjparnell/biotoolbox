...
sub _collect_bigbed_data {
	...
	foreach my $feature (@bed_features) {
		...
		if ($feature->has_tag('bigbedfile') ) {
			...
			# get wigfile name
			my @bedfiles = $feature->get_tag_values('bigbedfile');
			my $bedfile = shift @bedfiles;
			...
			my @features = $bb->get_features_by_location(
				$region->seq_id, 
				$region->start, 
				$region->end
			);
			...
			foreach my $feature (@features) {
				...
				if (
					...
					if ($feature->start == $feature->end) {
						...
						$position = int( 
							( ($feature->start + $feature->end) / 2) + 0.5
						);
					}
					...
					if ($method eq 'score') {
						...
						push @{ $bed_data{$position} }, $feature->score + 0;
					}
					elsif ($method eq 'count') {
						$bed_data{$position} += 1;
					}
						...
						push @{ $bed_data{$position} }, $feature->length;
					}
				}
			}
		}
...
bigbed file (.bb) that is referenced in the database. Typically, a single 
...
read using the Bio::DB::BigBed module, and the values extracted from the 
region of interest. 
...
The subroutine is passed five or more arguments in the following order:
    ...
    5) One or more database feature objects that contain the reference 
       ...
       which has the path to the BigBed file.
