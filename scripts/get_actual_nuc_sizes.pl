...
Optionally specify the minimum size of fragment to include 

=item --max <integer>

Optionally specify the maximum size of fragment to include 
...
The input files are BAM files as described by the Samtools 
project (http://samtools.sourceforge.net). The input file should be sorted and 
indexed prior to sizing. 
