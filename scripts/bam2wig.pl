...
" FATAL: number of scale values does not equal number of bam files!\n";
	...
	if ( $paired and $splice ) {
		if ($use_span) {
			print
" switching mode to smart paired coverage with paired and splice enabled\n";
			$use_span    = 0;
			$use_smartpe = 1;
		}
		elsif ($use_smartpe) {
...
sub determine_shift_value {
	...
	if ($model) {
		write_model_file(
			$best_value,            \@trimmed_f_profile,
			\@trimmed_r_profile,    \@trimmed_shifted_profile,
			\@trimmed_r_values,     \@trimmed_regions,
...
sub calculate_strand_correlation {
	...
	return [
		\@shift_values, \@f_profile, \@r_profile, \@shifted_profile,
		\@all_r_values, \@regions,   \@bestr_values
	];
}
...
sub scan_high_coverage {
	...
	foreach my $p ( keys %pos2depth ) {
		push @collected, $p
			if $pos2depth{$p} > $mincutoff and $pos2depth{$p} < $maxcutoff;
	}
	if ($verbose) {
...
sub write_model_file {
	my ( $value, $f_profile, $r_profile, $shifted_profile, $r_valuess,
		$regions, $region_shifts, $region_bestr )
		= @_;
...
sub open_wig_file {
	...
	my $fh = Bio::ToolBox->write_file( $name, $do_gz )
		or die " unable to open output wig file '$name'!\n";
...
" unable to find all the temporary binary children files! Only found %d, expected %d\n",
		...
		if ( any { $_ == 0 } @totals ) {
			die sprintf(
				" One or more bam files generated zero alignment counts!\n Counts: %s\n",
				join( ',', @totals ) );
		}
		...
		# otherwise we just normalize
			...
			foreach my $seq_id (@seq_list) {
				foreach my $strand (qw(f r)) {
					next unless defined $files{$seq_id}{$strand};
					normalize_bin_file( $files{$seq_id}{$strand}{0},
						$scale_factor, $seq_id );
				}
			}
		}
...
sub process_alignments_on_chromosome {
	...
	if ($verbose) {
		printf "  Converted %s $items on $seq_id in %d seconds\n",
			format_with_commas( $data->{f_count} + $data->{r_count} ),
			time - $chr_start_time;
		if ( $chrapply and $seq_id =~ /$chrapply/i ) {
...
sub normalize_bin_file {
	...
	# open file
	my $fh = Bio::ToolBox->read_file($file)
		or die " unable to read temporary $file!\n";
	$fh->binmode;
	...
	for ( my $pos = 0; $pos < $seq_bin_length; $pos += $long_window ) {
		...
		# check length
		my $len =
			  ( $pos + $long_window ) > $seq_bin_length
			? ( $seq_bin_length - $pos )
			: $long_window;
		...
		$chrom_data .=
			pack( "f*", map { $_ * $scale_factor } unpack( "$binpack*", $string ) );
	}
...
sub write_final_wig_file {
	...
	if ( scalar(@filelist) != $expected ) {
		die sprintf(
			" unable to find all the children wig files! Only found %d, expected %d\n",
			scalar(@filelist), $expected );
	}
...
sub write_bedgraph {
	...
	for ( my $pos = 0; $pos < $seq_bin_length; $pos += $window ) {
		...
		# check length
		my $len =
			( $pos + $window ) > $seq_bin_length ? ( $seq_bin_length - $pos ) : $window;
...
sub se_start {
	my ( $a, $data, $score ) = @_;
	if ( $a->reversed ) {
		$data->{f}->[ int( ( $a->calend - 1 ) / $bin_size ) - $data->{f_offset} ] +=
			$score;
	}
	else {
