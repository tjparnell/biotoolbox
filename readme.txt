...
These programs include the following capabilities
	...
	* Tools to prepare bioinformatic data (microarray & sequencing) into 
	  data formats (GFF v.3, wig, bigwig, bam) for loading into a 
...
In addition, some other libraries need to be installed which are required for the above to work.
	...
	* samtools (http://samtools.sourceforge.net), specifically libbam.a and libbam.h for Bio-samtools, but also the executables for generally working with bam files
	...
	* mysql (http://www.mysql.com) The Bio::DB::SeqFeature::Store database is configured to use a mysql database backend (although others could be used). On Mac OS X, this can also be installed through Fink.
	* 
...
if a required module is not found.
