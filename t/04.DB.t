...
is( min( keys %score1 ),
    -467, 'min 5prime relative position in positioned score hash' );
is( max( keys %score1 ),
    467, 'max 5prime relative position in positioned score hash' );
is( $score1{218}, 0.62,
...
is( min( keys %score1 ),
    -430, 'min 3prime relative position in positioned score hash' );
is( max( keys %score1 ),
    481, 'max 3prime relative position in positioned score hash' );
is( $score1{187}, 0.75,
...
is( scalar( keys %score1 ),
    39, 'number of keys extended relative positioned score hash' );
...
is( $score1{161}, 0.392,
    'score at position 161 in CDS region position scores' );
...
# print "CDS position_score is \n" . print_hash(\%score1);
is( $score1{322}, 0.461,
    'score at position 321 in CDS region position scores' );
is( $score1{2314}, 0.262,
    'score at position 2313 in CDS region position scores' );
...
undef $stream;
$Data = Bio::ToolBox::Data->new(
    file => File::Spec->catfile( $Bin, "Data", "sample.bed" ) );
