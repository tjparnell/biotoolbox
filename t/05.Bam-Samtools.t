...
# Test script for Bio::ToolBox::db_helper::bam
...
BEGIN {
	if ( eval { require Bio::DB::Sam; 1 } ) {
		...
		skip_all('Alternate module Bio::DB::Sam not available');
	}
	## no critic
...
isa_ok( $db, ['Bio::DB::Sam'], 'got a Bio::DB::Sam database object' );
...
isa_ok( $fdb, ['Bio::DB::Sam::Fai'], 'got a Sam fasta database object' );
my $seq = get_genomic_sequence( $fdb, 'chrI', 257, 275 );
is( $seq, 'ACCCTACCATTACCCTACC', 'fetched fasta sequence' );
unlink "$fasta.fai";
# there is not much more we can do with this object, unlike HTS
...
isa_ok( $segment, ['Bio::DB::Sam::Segment'], 'got a row Sam segment object' );
